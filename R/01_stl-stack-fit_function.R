stl_stack_pred <- function(data, model_list, meta_model, horizon) {
  # Data preprocessing ------------------------------------------------------
  set.seed(1406)
  ...
  stl_decomp <-
    stats::stl(
      x = ts(data = data$fires, frequency = 12),
      s.window = "per"
    )$time.series
  ...
  for (imf in seq(ncol(stl_decomp))) {
    IMF[[imf]] <-
      data.frame(TTR::lags(stl_decomp[, imf], lag))
  ...
  # create Observed dfs
  Obs <- TTR::lags(data$fires, lag)[, 1]
  Obs_train <- Obs[1:cut]
  Obs_test <- tail(Obs, n - cut)

  stl_df <- data.frame(data$fires, stl_decomp)
  colnames(stl_df) <- c("Obs", colnames(stl_decomp))
  ...
  # define objects
  step_stl_pred <- list()
  metrics_stl_train <- list()
  metrics_stl_test <- list()
  stack_model <- list()
  stack_params <- list()
  stack_pred <- list()
      ...
      if (hrz == 1) {
        ...
        for (p in 1:cut) {
          if (p %% hrz != 1) {
            ...
            if (hrz <= lag) {
              for (l in 1:(hrz - 1)) {
                Comp_train[[m]][[c]][p + l, l] <- PTRmo[[m]][p, c]
              }
            } else {
              for (l in 1:lag) {
                Comp_train[[m]][[c]][p + l, l] <- PTRmo[[m]][p, c]
        ...
        for (p in 1:(n - cut)) {
          if (p %% hrz != 1) {
            ...
            if (hrz <= lag) {
              for (l in 1:(hrz - 1)) {
                Comp_test[[m]][[c]][p + l, l] <- PTEmo[[m]][p, c]
              }
            } else {
              for (l in 1:lag) {
                Comp_test[[m]][[c]][p + l, l] <- PTEmo[[m]][p, c]
  ...
  {
    metrics_stl_train[[h]] <- matrix(nrow = dim(combs)[1], ncol = 4)
    metrics_stl_test[[h]] <- matrix(nrow = dim(combs)[1], ncol = 4)
    colnames(metrics_stl_train[[h]]) <- c("i", "MAE", "MAPE", "RMSE")
    colnames(metrics_stl_test[[h]]) <- colnames(metrics_stl_train[[h]])
    rownames(metrics_stl_train[[h]]) <- model_list
    rownames(metrics_stl_test[[h]]) <- rownames(metrics_stl_train[[h]])

    step_stl_pred_train <- matrix(nrow = dim(IMF_xtrain[[1]])[1], ncol = dim(combs)[1])
    step_stl_pred_test <- matrix(nrow = dim(IMF_xtest[[1]])[1], ncol = dim(combs)[1])
    step_stl_pred[[h]] <- matrix(nrow = n, ncol = dim(combs)[1])
  }

  for (i in 1:dim(combs)[1]) {
    step_stl_pred_train[, i] <- rowSums(PTRmo[[i]])
    step_stl_pred_test[, i] <- rowSums(PTEmo[[i]])

    ### Avoiding negative values
    for (j in 1:dim(step_stl_pred_train)[1]) {
      if (step_stl_pred_train[j, i] < 0) {
        step_stl_pred_train[j, i] <- 0
      }
    }
    for (j in 1:dim(step_stl_pred_test)[1]) {
      if (step_stl_pred_test[j, i] < 0) {
        step_stl_pred_test[j, i] <- 0
      }
    }

    step_stl_pred[[h]][, i] <- c(
      step_stl_pred_train[, i],
      step_stl_pred_test[, i]
    )

    # metrics
    stl_step_mae_train <- MAE(step_stl_pred_train[, i], Obs_train)
    stl_step_mape_train <- mape(step_stl_pred_train[, i], Obs_train)
    stl_step_rmse_train <- RMSE(step_stl_pred_train[, i], Obs_train)

    stl_step_mae_test <- MAE(step_stl_pred_test[, i], Obs_test)
    stl_step_mape_test <- mape(step_stl_pred_test[, i], Obs_test)
    stl_step_rmse_test <- RMSE(step_stl_pred_test[, i], Obs_test)

    metrics_stl_train[[h]][i, ] <- c(
      i,
      stl_step_mae_train,
      stl_step_mape_train,
      stl_step_rmse_train
    )
    metrics_stl_test[[h]][i, ] <- c(
      i,
      stl_step_mae_test,
      stl_step_mape_test,
      stl_step_rmse_test
  ...
  step_stl_pred[[h]] <- cbind(Obs, step_stl_pred[[h]])
  colnames(step_stl_pred[[h]]) <- c("Obs", model_list)

  # Stacking ----------------------------------------------------------------
  cat("\nStacking\n")
  stack_data <- step_stl_pred[[h]]
  ...
  metrics_stack_train[[h]] <- c(
    stack_mae_train,
    stack_mape_train,
    stack_rmse_train
  )
  metrics_stack_test[[h]] <- c(
    stack_mae_test,
    stack_mape_test,
    stack_rmse_test
  )

  predictions[[h]] <- cbind(Obs, stack_pred[[h]], step_stl_pred[[h]][, -1])
  ...
  names(step_stl_pred) <- paste0(horizon, "-step")
  names(metrics_stl_train) <- names(step_stl_pred)
  names(metrics_stl_test) <- names(step_stl_pred)
  names(metrics_stack_train) <- names(step_stl_pred)
  names(metrics_stack_test) <- names(step_stl_pred)
  names(stack_pred) <- names(step_stl_pred)
  names(predictions) <- names(step_stl_pred)
  names(errors) <- names(step_stl_pred)

  results <- list(
    Predictions = predictions,
    Decomp_Metrics = metrics_stl_test,
