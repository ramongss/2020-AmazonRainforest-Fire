Plot2<-function(date,Obs,Pred1,Pred2)
{
  data <- data.frame(as.vector(unlist(data.frame(Obs,Pred1,Pred2))),
                     rep(c("Observed","One-month-ahead","Two-months-ahead"),
                         each=dim(Obs)[1]),
                     rep(date),
                     rep(c(1:dim(Obs)[1]),times=3))
  ...
  g2 <- g2 + scale_color_manual(values=c("#000000","#0000ff","#FF0000"))
  g2 <- g2 + annotate("text", x = data$Frequency[24], y=max(Obs), label = "Training set", size=6)
  g2 <- g2 + annotate("text", x = data$Frequency[196], y=max(Obs), label = "Test set", size=6)
  g2 <- g2 + geom_vline(aes(xintercept=as.numeric(Frequency[171])),size = 1, color ="black")
